...
{
    Task_Timer.obj = lv_obj_create(lv_scr_act(),NULL);
    lv_obj_set_style_local_border_opa(Task_Timer.obj, LV_OBJ_PART_MAIN, LV_STATE_DEFAULT, LV_OPA_TRANSP);
    lv_obj_set_style_local_bg_color(Task_Timer.obj, LV_OBJ_PART_MAIN, LV_STATE_DEFAULT, LV_COLOR_BLACK);
    lv_obj_set_size(Task_Timer.obj, LV_HOR_RES_MAX, LV_VER_RES_MAX-30);
    lv_obj_align(Task_Timer.obj, NULL, LV_ALIGN_CENTER, 0, 25);
...
static void title_init()
{
    ...
    //添加字体
    title_label = lv_label_create(Task_Timer.obj, NULL);
    ...
    lv_obj_align(title_label, Task_Timer.obj, LV_ALIGN_IN_TOP_MID, 0, 5);
...
static void redline_init()
{
    ...
    lv_obj_t* line = lv_line_create(Task_Timer.obj, NULL);
...
static void circle_init()
{
    ...
    for (int i = 0; i < Line_number; i++)
    {
        ...
        line_obj[i] = lv_line_create(Task_Timer.obj, NULL);
    ...
    time_label = lv_label_create(Task_Timer.obj, NULL);
...
static void rect_init()
{
    ...
    lv_obj_t* line1 = lv_line_create(Task_Timer.obj, NULL);
    ...
    lv_obj_t* line2 = lv_line_create(Task_Timer.obj, NULL);   
    ...
    for (int i = 0; i < 3; i++)
    {
        counter_label[i] = lv_label_create(Task_Timer.obj, NULL);
