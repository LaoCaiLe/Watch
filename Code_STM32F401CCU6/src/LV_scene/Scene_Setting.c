...
{
    Task_Setting.obj = lv_obj_create(lv_scr_act(),NULL);
    lv_obj_set_style_local_border_opa(Task_Setting.obj, LV_OBJ_PART_MAIN, LV_STATE_DEFAULT, LV_OPA_TRANSP);
    lv_obj_set_style_local_bg_color(Task_Setting.obj, LV_OBJ_PART_MAIN, LV_STATE_DEFAULT, LV_COLOR_BLACK);
    lv_obj_set_size(Task_Setting.obj, LV_HOR_RES_MAX, LV_VER_RES_MAX-30);
    lv_obj_align(Task_Setting.obj, NULL, LV_ALIGN_CENTER, 0, 25);
}
...
static void text_label_init()
{
    ...
    time_label[0] = lv_label_create(Task_Setting.obj, NULL); 
    lv_obj_add_style(time_label[0], LV_OBJ_PART_MAIN, &font_style);    

    time_label[1] = lv_label_create(Task_Setting.obj, time_label[0]); 
    time_label[2] = lv_label_create(Task_Setting.obj, time_label[0]); 
    ...
    lv_obj_add_style(Time_label[0], LV_OBJ_PART_MAIN, &font_style);
    for (int i = 1; i < 7; i++) 
        Time_label[i] = lv_label_create(Task_Setting.obj, Time_label[0]); 
    ...
    lv_obj_align(Time_label[2], NULL, LV_ALIGN_CENTER, 50, -58);
    lv_obj_align(Time_label[3], NULL, LV_ALIGN_CENTER, -25, -10);
    lv_obj_align(Time_label[4], NULL, LV_ALIGN_CENTER, 38, -10);
    lv_obj_align(Time_label[5], NULL, LV_ALIGN_CENTER, -48, 38);
...
{
    rect_obj[0] = lv_obj_create(Task_Setting.obj, NULL);
    ...
    for (int i = 1; i < 8; i++)
        rect_obj[i] = lv_obj_create(Task_Setting.obj, rect_obj[0]);
...
void Setting_KeyFir_Func()
{
    ...
    switch(index)
    {
        ...
        case 2:
            ...
            val++;
            val %= 60;
            RTC_Set_Time(RTC_TimeStruct.RTC_Hours, RTC_TimeStruct.RTC_Minutes, val, RTC_H12_AM); //设置时间
            lv_label_set_text_fmt(Time_label[2], "%02d", RTC_TimeStruct.RTC_Seconds);
            break;
