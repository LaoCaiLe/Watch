...
IMG_t img_array[] =
{
    ...
    {.img_addr = &img_alarm,    .title = "Alarm",     .task_t = &Task_Alarm },
    {.img_addr = &img_lighting, .title = "Lighting",  .task_t = &Task_Lighting},
    {.img_addr = &img_game,     .title = "Game",      .task_t = NULL },
    {.img_addr = &img_timer,    .title = "Timer",     .task_t = &Task_Timer },
    {.img_addr = &img_setting,  .title = "Setting",   .task_t = &Task_Setting }
};
...
{
    Task_Select.obj = lv_obj_create(lv_scr_act(), NULL);
    lv_obj_set_style_local_border_opa(Task_Select.obj, LV_OBJ_PART_MAIN, LV_STATE_DEFAULT, LV_OPA_TRANSP);
    lv_obj_set_style_local_bg_color(Task_Select.obj, LV_OBJ_PART_MAIN, LV_STATE_DEFAULT, LV_COLOR_BLACK);
    lv_obj_set_size(Task_Select.obj, LV_HOR_RES_MAX, LV_VER_RES_MAX);
    lv_obj_align(Task_Select.obj, NULL, LV_ALIGN_CENTER, 0, 0);
}
...
{    
    ...
    //添加字体
    title_label = lv_label_create(Task_Select.obj, NULL);
    ...
    lv_obj_align(title_label, Task_Select.obj, LV_ALIGN_IN_TOP_MID, 0, 5);
...
void redline_init()
{
    ...
    lv_obj_t* line1 = lv_line_create(Task_Select.obj, NULL);
...
void objmask_init()
{    
    om = lv_objmask_create(Task_Select.obj, NULL);
...
void img_init()
{
    ...
    // lv_obj_align(img_obj, Task_Select.obj, LV_ALIGN_CENTER, 0, 30); 
...
void Select_KeyFir_Func()
{
    if(img_array[Index].task_t!=NULL)
    {
        ...
        MenuTask = img_array[Index].task_t;
...
void Select_KeyCancel_Func()
{
    ...
    lv_obj_move_foreground(Task_MainScence.obj);
}