...
{
    Task_Lighting.obj = lv_obj_create(lv_scr_act(),NULL);
    lv_obj_set_style_local_border_opa(Task_Lighting.obj, LV_OBJ_PART_MAIN, LV_STATE_DEFAULT, LV_OPA_TRANSP);
    lv_obj_set_style_local_bg_color(Task_Lighting.obj, LV_OBJ_PART_MAIN, LV_STATE_DEFAULT, LV_COLOR_BLACK);
    lv_obj_set_size(Task_Lighting.obj, LV_HOR_RES_MAX, LV_VER_RES_MAX-30);
    lv_obj_align(Task_Lighting.obj, NULL, LV_ALIGN_CENTER, 0, 24);
...
static void title_init()
{
    ...
    //添加字体
    title_label = lv_label_create(Task_Lighting.obj, NULL);
    ...
    lv_obj_align(title_label, Task_Lighting.obj, LV_ALIGN_IN_TOP_MID, 0, 5);
...
static void redline_init()
{
    ...
    lv_obj_t* line1 = lv_line_create(Task_Lighting.obj, NULL);
...
static void circle_num_init()
{
	...
	/*画圆初始化*/
	arc = lv_arc_create(Task_Lighting.obj,NULL);
	...
	//设置文本标签
	num_label = lv_label_create(Task_Lighting.obj, NULL);
